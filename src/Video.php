...
namespace Opendisk\Bing;

use Opendisk\Bing\Bing;
...
class Video extends Bing
{
    ...
    public function getContent()
    {
        ...
        $url = "https://www.bing.com/videos/asyncv2?q=" . $q . "&async=content&first=0&count=35&dgst=RowIndex_u9*ColumnIndex_u3*TotalWidth_u864*OrdinalPosition_u35*ThumbnailWidth_u260*HeroContainerWidth_u1125*HeroContainerHeight_u275*HeroOnPage_b0*SlidesGridOnPage_b0*arn_u3*ayo_u869*cry_u2729*&IID=video.1&SFX=2&IG=" . $token . "&CW=1385&CH=620&bop=88&form=QBFVBS";
        $response = @file_get_contents($url);
    ...
    {
        $results = $this->crawler->filter(".dg_u .vrhdata")->each(function (Crawler $node, $i) {
